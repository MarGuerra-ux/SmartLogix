...
function InventoryPage() {
  ...
  async function handleCategorySubmit(event) {
    ...
    if (categoryAction === "edit") {
      result = await updateCategory(categoryForm.id, {
        name: categoryForm.name,
        description: categoryForm.description,
        updated_at: new Date().toISOString(),
      });
    ...
    if (categoryAction === "delete") {
      const confirmDelete = window.confirm(
        `¿Seguro que deseas eliminar la categoría "${categoryForm.name}"?`
      );

      if (!confirmDelete) return;

      result = await deleteCategory(categoryForm.id);
    }
    ...
    <section className="module-page">
      <div className="module-header module-header-row">
        <div>
          ...
          <p>Ingreso, control y consulta de mercadería conectada a Supabase.</p>
        </div>
          ...
          <div className="dropdown-wrapper">
            ...
            {isCategoryMenuOpen && (
              <div className="dropdown-menu">
                ...
                <button onClick={() => openCategoryModal("delete")}>
                  Eliminar categoría
                </button>
              </div>
            )}
        ...
        <tbody>
          {products.map((product) => (
            <tr key={product.id}>
              ...
              <td>{product.stock_units} unidades</td>
              <td>${Number(product.unit_price).toLocaleString("es-CL")}</td>
      ...
      {isModalOpen && (
        <div className="modal-backdrop">
          <article className="modal-card">
            ...
            <form className="module-form modal-form" onSubmit={handleSubmit}>
        ...
        <div className="modal-backdrop">
          <article className="modal-card small-modal-card">
            <div className="modal-header">
              <div>
                <h3>
                  {categoryAction === "create" && "Agregar categoría"}
                  {categoryAction === "edit" && "Editar categoría"}
                  {categoryAction === "delete" && "Eliminar categoría"}
                </h3>
                <p>Administración independiente de categorías del inventario.</p>
              </div>
            ...
            >
              <div className="form-grid single-column-form">
                ...
                {categoryAction === "delete" && (
                  <p className="delete-warning">
                    Selecciona una categoría para eliminarla del sistema.
                  </p>
                )}
              </div>
              ...
              <div className="modal-actions">
                ...
                >
                  {categoryAction === "create" && "Agregar categoría"}
                  {categoryAction === "edit" && "Guardar cambios"}
                  {categoryAction === "delete" && "Eliminar categoría"}
                </button>
